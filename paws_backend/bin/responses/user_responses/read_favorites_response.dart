...
readFavoritesHandler(Request req) async {
  try {
    ...
    for (int i = 0; i < favindex.length; i++) {
      final readPost =
          await supabase.from("posts").select().eq("post_id", favindex[i]);

      final readAnimals =
          await supabase.from("animals").select().eq("post_id", favindex[i]);
      post.add(readPost);
    ...
    return Success().responseMessage(
      message: 'your favorites posts',
      data: {"favorites": favorite.toString()},
    );
  } catch (error) {
    print(error);
